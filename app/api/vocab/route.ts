...
"""`;
    ...
    const response = await client.responses.create({
      ...
      // 語彙数が多い教材（最大40個）でも出力が途中で切れないよう、出力トークン数を確保する
    ...
    console.error("Vocabulary extraction error:", e?.message, detail);
    const hasDetail = detail.status || detail.code || detail.param;
    const message = hasDetail
      ? (e.message || "Vocabulary extraction error") + " (status:" + (detail.status ?? "-") + " code:" + (detail.code ?? "-") + " param:" + (detail.param ?? "-") + ")"
      : e.message || "Vocabulary extraction error";
